...
if (isset($_POST['tgl_transaksi'])) {
    ...
    $sql =
        "SELECT DATE_FORMAT(np.tgl_transaksi,'%Y-%m') AS periode_transaksi, SUM(np.biaya_admin) as total_admin, vst.perusahaan
...
if (isset($_POST['tgl_transaksi_detail'])) {
    ...
    $sql =
        "SELECT DATE_FORMAT(np.tgl_transaksi,'%Y-%m') AS periode_transaksi, (np.biaya_admin) as total_admin, vst.perusahaan
        FROM nota_penjualan np
        INNER JOIN visit vst ON vst.id=np.visit_id
        WHERE np.tgl_transaksi LIKE ?
        && vst.perusahaan IS NOT NULL";
